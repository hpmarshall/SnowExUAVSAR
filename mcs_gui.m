...
S.ddSARProd = addDropdown(S.pSAR, {'Coherence', 'Unwrapped phase', 'Wrapped phase (angle)', ...
    'Wrapped phase (amplitude)', 'Atm delay difference'}, ...
    {'coherence', 'unwrapped_phase', 'wrapped_angle', 'wrapped_amplitude', 'atm_delay_diff'}, ...
    @(varargin) refreshSAR());
...
% ================= nested callbacks (share S via closure) =================
    ...
    function refreshSAR()
        ...
        names = containers.Map({'coherence', 'unwrapped_phase', 'wrapped_angle', ...
            'wrapped_amplitude', 'atm_delay_diff'}, {'Coherence', 'Unwrapped phase (rad)', ...
            'Wrapped phase (rad)', 'Wrapped amplitude', 'Atm delay diff (rad)'});
        maps = containers.Map({'coherence', 'unwrapped_phase', 'wrapped_angle', ...
            'wrapped_amplitude', 'atm_delay_diff'}, {cmapSeq(), cmapDiverging(), hsv(256), ...
            cmapSeq(), cmapDiverging()});
        signed = ismember(prod, {'unwrapped_phase', 'atm_delay_diff'});  % zero-centered products
