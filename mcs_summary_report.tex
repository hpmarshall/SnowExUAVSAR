...
\large Selected InSAR pairs for UCLA Data Assimilation testing}
...
\caption{Mores Creek Summit SNOTEL (637:ID:SNTL) daily SWE (left axis) and mean air
temperature (right axis, purple, dashed gray 0\,$^\circ$C line) for the two campaign
winters. Vertical lines: UAVSAR flights (line 23205 solid red, line 05208 dotted orange;
the two lines flew the same days in 2021). Dashed green: QSI lidar snow-depth surveys
(2020-02-09 and 2021-03-15). Double-headed arrows (bottom panel): the intervals of the
selected pairs A--F of Table~\ref{tab:top}, each labeled with its polarization-mean
correlation against the season-matched lidar; Pair F (gray) is the zero-change example.}
\label{fig:snotel}
\end{figure}

\section{Recommended pairs}

Table~\ref{tab:top} lists the five ranked pairs (labeled A--E, shown as interval arrows
in Figure~\ref{fig:snotel}) plus one additional example (Pair F). The rank
...
\label{tab:top}
\medskip
\resizebox{\textwidth}{!}{\input{tex_fragments/rank_top}}
\end{table}

Two features strengthen the top choices: Pair A (pair 8) correlates nearly equally with
the 2020 lidar ($+0.650$) and the 2021 lidar ($+0.637$), evidence that the accumulation
pattern it captures is the persistent, terrain-controlled one a model precipitation field
should inherit; and Pairs D/E (pairs 5/6) sample the same early-March snowfall from
opposite headings, giving a built-in cross-view consistency check.
...
\item \texttt{mcs\_top\_pairs\_subset.nc} (1.2\,GB NetCDF4): the six pairs of
      Table~\ref{tab:top} in rank order (\texttt{rank\_note} coordinate carrying the
      A--F letters), with
...
\item \texttt{mcs\_subset\_gui.m} (+ \texttt{mcs\_annotations.json}): standalone
      MATLAB viewer for the subset file --- pair (A--F), polarization, product and
      lidar-date dropdowns over four linked panels.
\end{itemize}

\section{Software used}
This workflow used \texttt{asf\_search} to pull UAVSAR data from ASF where possible, and custom scripts to download from the JPL UAVSAR website where needed.  To convert from the binary custom UAVSAR format and calculate local incidence angles from the UAVSAR look vectors, we used \texttt{uavsar\_pytools}.  To estimate the atmospheric phase delay we used \texttt{phase-o-matic}, and to perform the custom unwrapping we used \texttt{SNAPHU}. 
...
\label{tab:appendix}
\medskip
\resizebox{\textwidth}{!}{\input{tex_fragments/rank_appendix}}
\end{table}
